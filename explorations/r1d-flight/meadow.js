...
var MEADOW = (function () {
  ...
  var P = {
    ...
    /* ── how much margin the octave fades keep against their own limit ────
       Every noise pass here fades an octave out as the screen cell grows
       toward the octave's wavelength, and every one of them used to hold full
       strength right up to `foot = scale/2` — one cell for every half cycle,
       which is Nyquist exactly, and only reached zero at one sample per cycle,
       which is past it.

       That is defensible for a still frame and wrong for a moving one. At two
       samples a cycle the phase the noise is sampled at is decided by where
       the camera happens to be standing, so a fraction of a cell of movement
       swings the value across the noise's whole range. The ground is then
       drawn through eight colours and a 4×4 dither, where a swing that size
       does not shade differently, it flips the cell to another colour outright
       — and thousands of cells flipping in a region every frame is the boiling
       that reads as blobs crawling over the peaks.

       So the fades keep margin in hand rather than running to the limit. The
       finest octave retires a little nearer the camera than it used to, which
       costs some texture in the middle distance and buys ground that holds
       still.

       The knee is sharp and it is worth knowing where: measured on two
       consecutive frames with the camera nudged a fraction of a cell, the
       share of near-field pixels that change colour outright runs 15.8% at
       0.50, 0.55% at 0.35, 0.44% at 0.25. Almost all of the boiling goes in
       the first step, so this sits just past the knee rather than deep into
       it — no more texture is given up than the artefact costs. */
    aa:         0.30,  /* fraction of an octave's scale it stays whole to  */
  ...
  function relief(px, pz, foot, range, sunX, sunZ) {
    ...
    var s = P.reliefScale;
    if (foot >= s * P.reliefStep * P.aa * 2) return 0;

    var v = 0, i, sc, fade, e, gx, gz, w;
    for (i = 0; i < 2; i++) {
      sc = i === 0 ? s : s * P.reliefStep;
      fade = 1 - smooth01(foot / (sc * P.aa) - 1);
      if (fade <= 0) continue;
      e = sc * 0.28;
  ...
  function grain(px, pz, foot) {
    ...
    if (foot >= P.clumpScale * P.aa * 2) return 0;

    var g = 0, w = 0;

    var fadeA = 1 - smooth01(foot / (P.grainScale * P.aa) - 1);
    if (fadeA > 0) {
      g += vnoise(px / P.grainScale, pz / P.grainScale) * fadeA;
      w += fadeA;
    }
    var fadeB = 1 - smooth01(foot / (P.clumpScale * P.aa) - 1);
    if (fadeB > 0) {
  ...
  function rock(px, pz, foot) {
    if (foot >= P.rockClumpScale * P.aa * 2) return 0;

    var g = 0, w = 0;

    var fadeA = 1 - smooth01(foot / (P.rockScale * P.aa) - 1);
    if (fadeA > 0) {
      g += vnoise(px / P.rockScale, pz / P.rockScale) * fadeA;
      w += fadeA;
    }
    var fadeB = 1 - smooth01(foot / (P.rockClumpScale * P.aa) - 1);
    if (fadeB > 0) {
